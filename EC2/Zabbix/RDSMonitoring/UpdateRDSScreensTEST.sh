...
RDSES=`./GetRDSInstancesTags.sh | awk -F"," '{print $1","$2}' | sort | uniq | grep qa`
ROW=0
COL=0
...
do
#  echo $ROW $COL $graph
  cat Items/UpdateRDSScreenTEST.json | sed "s/\$ROW/$ROW/g;s/\$COL/$COL/g;s/\$GRAPH/$graph/g" >Items/tmp/UpdateRDSScreenTEST.json
  curl -H "Content-Type: application/json" --data @Items/tmp/UpdateRDSScreenTEST.json http://10.2.104.138/zabbix/api_jsonrpc.php
